...
STAGE_STALL_THRESHOLD = {
    ...
    "JBP Signed": 35, "Live Campaign": 60, "Renewal": 30,
}

# PO Received / Budget Confirmed is excluded from at-risk flagging entirely —
# once the budget is actually confirmed, the deal isn't "at risk" of falling
# through commercially the way an unsigned or unfunded deal is.
def flag_at_risk(row):
    if row["stage"] == "PO Received / Budget Confirmed":
        return False
...
st.caption(
    ...
    "Negotiation more than 30 days, JBP Signed more than 35 days, Live Campaign more than 60 days, "
    "Renewal more than 30 days. PO Received / Budget Confirmed is excluded — once the budget is "
    "actually confirmed, the deal isn't commercially at risk the way an unfunded one is. At first, "
    "these thresholds are set from experience and judgement. In future, once enough deal history has "
    "been generated, this would convert to a predictive model built on a real data pipeline — trained "
    "on actual won/lost/stalled outcomes — to flag at-risk deals based on learned patterns rather than "
    "fixed rules."
)
