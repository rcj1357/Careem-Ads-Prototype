...
# Fixed colors for the New vs. Renewal split (deliberately different from
# Endemic/Non-endemic's greens, so the two splits are never visually confused).
DEAL_TYPE_COLORS = {"New / Growth": CAREEM_GREEN, "Renewal": MIDNIGHT_BLUE}
...
STAGE_STALL_THRESHOLD = {
    "Prospecting": 40, "Proposal Sent": 28, "Negotiation": 30, "JBP Signed": 35,
}

# Confirmed/executing stages are excluded from at-risk flagging entirely — once
# the budget is confirmed (PO Received), the campaign is running (Live Campaign),
# or the client has renewed (Renewal), the deal isn't "at risk" of falling through
# commercially the way an unsigned or unfunded deal is. Days-in-stage there just
# reflects how long it's been running/confirmed, not stalling.
CONFIRMED_NOT_AT_RISK = ["PO Received / Budget Confirmed", "Live Campaign", "Renewal"]

def flag_at_risk(row):
    if row["stage"] in CONFIRMED_NOT_AT_RISK:
        return False
...
# ---------------------------------------------------------------------------
# New vs. Renewal — a renewal is an existing client keeping their contract
# going, not a new client won for the first time, so it's worth tracking
# separately from new/growth pipeline even though both count toward target.
# Note: this is a stage-based proxy (only the "Renewal" stage counts as
# renewal) since the dummy data has no separate new-vs-existing account
# flag — a real version of this would tag deals at the account level.
# ---------------------------------------------------------------------------
scope["deal_type"] = scope["stage"].apply(lambda s: "Renewal" if s == "Renewal" else "New / Growth")
renewal_share = scope.loc[scope["deal_type"] == "Renewal", "pipeline_value_usd"].sum() / total_pipeline

c5, c6 = st.columns([1, 1])
with c5:
    by_type = scope.groupby("deal_type")["pipeline_value_usd"].sum().reset_index()
    fig_type = px.pie(by_type, names="deal_type", values="pipeline_value_usd", hole=0.55,
                       title="New vs. Renewal Mix", color="deal_type", color_discrete_map=DEAL_TYPE_COLORS)
    fig_type.update_traces(hovertemplate="%{label}: $%{value:,.0f} (%{percent})<extra></extra>")
...
st.caption(
    ...
    "Live Campaign, and Renewal are all excluded — once the budget is confirmed, the campaign is "
    "running, or the client has renewed, days-in-stage just reflects how long that's been true, not "
    "stalling, so it isn't commercially at risk the way an unfunded deal is. At first, these "
